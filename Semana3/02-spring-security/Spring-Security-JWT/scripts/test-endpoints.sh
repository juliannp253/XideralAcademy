...
BASE="http://localhost:8072/api/pokemons"
...
probar "POST con token de john (EMPLOYEE)"  403 -H "Authorization: Bearer $TJ" -X POST -H 'Content-Type: application/json' -d '{"name":"Mewtwo","type":"Psíquico","level":50}' "$BASE"
...
probar "HTTP Basic contra /api/pokemons"    401 -u susan:test123 "$BASE"
...
FALSO="$(echo "$TJ" | cut -d. -f1).$(echo -n '{"iss":"security-jwt","sub":"john","exp":9999999999,"roles":["ROLE_ADMIN"]}' | base64 -w 0 | tr -d '=' | tr '/+' '_-').$(echo "$TJ" | cut -d. -f3)"
...
NUEVO=$(curl -s -H "Authorization: Bearer $TM" -X POST "$BASE" -H "Content-Type: application/json" \
  -d '{"name":"TempPokemon","type":"Agua","level":20}')
echo "$NUEVO"
